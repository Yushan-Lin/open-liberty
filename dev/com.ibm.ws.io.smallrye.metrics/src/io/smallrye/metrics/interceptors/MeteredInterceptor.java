...
public class MeteredInterceptor {
    ...
    private <E extends Member & AnnotatedElement> Object meteredCallable(InvocationContext context, E element) throws Exception {
        Set<MetricID> ids = ((MetricsRegistryImpl) registry).getMemberToMetricMappings().getMeters(new CDIMemberInfoAdapter<>().convert(element));
        ...
        ids.stream().map(metricID -> {
            ...
            if (metric == null) {
                throw new IllegalStateException(MessageFormat.format("No metric of type %s and ID %s found in registry", MetricType.METERED, metricID));
            }
            return metric;
        }).forEach(Meter::mark);
