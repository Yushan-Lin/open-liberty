...
public class SimplyTimedInterceptor {
    ...
    private <E extends Member & AnnotatedElement> Object timedCallable(InvocationContext invocationContext, E element) throws Exception {
        Set<MetricID> ids = ((MetricsRegistryImpl) registry).getMemberToMetricMappings().getSimpleTimers(new CDIMemberInfoAdapter<>().convert(element));
        if (ids == null || ids.isEmpty()) {
            throw new IllegalStateException(MessageFormat.format("No metric mapped for %s", element));
        }
        List<SimpleTimer.Context> contexts = ids.stream().map(metricID -> {
            SimpleTimer metric = registry.getSimpleTimers().get(metricID);
            if (metric == null) {
                throw new IllegalStateException(MessageFormat.format("No metric of type %s and ID %s found in registry", MetricType.SIMPLE_TIMER, metricID));
            }
            return metric;
