...
/* package-private */ class SeMetricName implements MetricName {
    ...
    public String of(InjectionPoint ip) {
        ...
        if (annotated instanceof AnnotatedMember) {
            ...
            throw new UnsupportedOperationException(MessageFormat.format("Unable to retrieve metric name for injection point %s", ip));
        }
    }
    ...
    private String getParameterName(AnnotatedParameter<?> parameter) {
        try {
            ...
            if ((Boolean) Parameter.getMethod("isNamePresent").invoke(param)) {
                ...
                throw new UnsupportedOperationException(MessageFormat.format("Unable to retrieve name for parameter %s", param));
            }
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException | ClassNotFoundException cause) {
            throw new UnsupportedOperationException(MessageFormat.format("Unable to retrieve name for parameter %s", parameter));
        }
    }
}
