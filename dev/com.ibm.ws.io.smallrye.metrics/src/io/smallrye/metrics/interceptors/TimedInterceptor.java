...
public class TimedInterceptor {
    ...
    private <E extends Member & AnnotatedElement> Object timedCallable(InvocationContext invocationContext, E element) throws Exception {
        Set<MetricID> ids = ((MetricsRegistryImpl) registry).getMemberToMetricMappings().getTimers(new CDIMemberInfoAdapter<>().convert(element));

        if (ids == null || ids.isEmpty()) {
            throw new IllegalStateException(MessageFormat.format("No metric mapped for %s", element));
        }
        List<Timer.Context> contexts = ids.stream().map(metricID -> {
            Timer metric = registry.getTimers().get(metricID);
            if (metric == null) {
                throw new IllegalStateException(MessageFormat.format("No metric of type %s and ID %s found in registry", MetricType.TIMER, metricID));
            }
            return metric;
