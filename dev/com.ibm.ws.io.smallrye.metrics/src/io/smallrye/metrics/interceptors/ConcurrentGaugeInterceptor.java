...
public class ConcurrentGaugeInterceptor {
    ...
    private <E extends Member & AnnotatedElement> Object concurrentCallable(InvocationContext context, E element) throws Exception {
        Set<MetricID> ids = ((MetricsRegistryImpl) registry).getMemberToMetricMappings().getConcurrentGauges(new CDIMemberInfoAdapter<>().convert(element));
        if (ids == null || ids.isEmpty()) {
            throw new IllegalStateException(MessageFormat.format("No metric mapped for %s", element));
        }
        List<org.eclipse.microprofile.metrics.ConcurrentGauge> metrics = ids.stream().map(metricID -> {
            org.eclipse.microprofile.metrics.ConcurrentGauge metric = registry.getConcurrentGauges().get(metricID);
            if (metric == null) {
                throw new IllegalStateException(MessageFormat.format("No metric of type %s and ID %s found in registry", MetricType.CONCURRENT_GAUGE, metricID));
            }
            return metric;
