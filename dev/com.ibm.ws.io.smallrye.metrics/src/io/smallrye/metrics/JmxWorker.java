...
public class JmxWorker {
                ...
                try {
                    ...
                    for (ObjectName oName : objNames) {
                        ...
                        for (final Entry<String, String> keyHolder : keyHolders.entrySet()) {
                            ...
                            newTags = newTags.stream().map(originalTag -> new Tag(originalTag.getTagName(), originalTag.getTagValue().replaceAll(Pattern.quote(keyHolder.getKey()),
                                                                                                                                                 keyValue))).collect(Collectors.toList());
                        }

                        String newObjectName = oName.getCanonicalName() + "/" + attName;

                        ExtendedMetadata newEntryMetadata = new ExtendedMetadata(newName, newDisplayName, newDescription, entry.getMetadata().getTypeRaw(), entry.getMetadata().getUnit(), newObjectName, true);
                        ExtendedMetadataAndTags newEntry = new ExtendedMetadataAndTags(newEntryMetadata, newTags);
                        result.add(newEntry);
                    }
                    toBeRemoved.add(entry);
                } catch (MalformedObjectNameException e) {
                    throw new IllegalStateException("Malformed object name");
                }
            }
        }
    ...
    private Map<String, String> findComponentPairsWithPlaceholders(ObjectName objectName) {
        return objectName.getKeyPropertyList().entrySet().stream().filter(entry -> entry.getValue().matches(PLACEHOLDER + "(\\d)?+")).collect(Collectors.toMap(
                                                                                                                                                               Entry::getValue,
                                                                                                                                                               Entry::getKey));
    }
}
